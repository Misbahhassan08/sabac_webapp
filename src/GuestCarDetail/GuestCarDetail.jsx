...
const GuestCarDetail = () => {
  ...
  const guestId = location.state?.guestId; // ✅ Get guest_id from navigation state
  ...
  const handleSubmit = async (event) => {
    ...
    // Ensure guest details exist if adding as a guest
    // if (!guestDetails) {
    //   console.error("Guest details not found!");
    //   alert("Guest details are required to add a car!");
    //   return; // Stop submission if guestDetails are missing
    // }
    if (!guestId) {
      console.error("Guest ID not found!");
      alert("Guest ID is required to submit car details.");
      return; // Stop submission if guestId is missing
    }

    // Log formData before submission to check if photos are included
    // console.log("Form data before submission:", formData);
    console.log("Submitting car details with Guest ID:", guestId);
    ...
    const carData = {
      ...
      added_by: "guest", // ✅ Ensure it's never null
    };

    try {
      const response = await axiosInstance.post(
        urls.guestAddCarDetail,
        carData
      );
      ...
      // Store car ID for inspector selection step
      localStorage.setItem("saler_car_id", car_id);
      setFormdata({
        car_name: "",
        company: "",
        color: "",
      ...
      navigate("/guest-inspector-list", { state: { car_id } });
    } catch (error) {
      console.error(
