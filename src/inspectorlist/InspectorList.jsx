...
function InspectorList() {
  ...
  console.log("Car id at inspector list", saler_car_id);
  ...
  const navigateToFreeSlot = (inspector) => {
    navigate("/inspectorfreeslot", { state: { inspector, saler_car_id } });
  };
  ...
  const handleSelect = (id) => {
    setSelectedInspector(selectedInspector === id ? null : id);
  };

  const linkCarandInspector = async (inspector_id) => {
    if (!saler_car_id) {
    ...
    if (!inspector_id) {
      alert("Inspector ID is missing. Please select an inspector.");
      return;
    }
    ...
    try {
      ...
      console.error(
        "Error linking inspector:",
        error.response?.data || error.message || error
      );
  
      alert(
        error.response?.data?.error || "Failed to link inspector. Please try again."
      );
    }
  };
          ...
          <Box key={inspector.id}>
            ...
            {selectedInspector === inspector.id && (
              ...
              >
                <Button
                  onClick={() => {
                    openWhatsapp(inspector.phone_number); // Open WhatsApp
                    linkCarandInspector(inspector.id); // Link Inspector to Car
                  }}
                  variant="contained"
                  color="primary"
                >
